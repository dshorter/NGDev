...
namespace BLToolkit.Data
{
	...
	public partial class DbManager: Component
	{
		...
		public void Close()
		{
			...
			{
				ExecuteOperation(OperationType.CloseConnection, _connection.Close); //!!! B&V
				_connection = null;
			}
			...
			object obj)
		{
			...
			foreach (IDbDataParameter parameter in Command.Parameters)
			{
                ...
                {   // BV Changes
                    ...
                    object newVal = _mappingSchema.ConvertChangeType(parameter.Value, dest.GetFieldType(ordinal));
        			IMapDataSource ds = dest as IMapDataSource;
                    bool bSet = true;
                    if (ds != null)
                    {
                        object oldVal = ds.GetValue(obj, ordinal);
                        if (oldVal != null && newVal != null)
                        {
                            if (oldVal is IComparable && newVal is IComparable)
                            {
                                if ((oldVal as IComparable).CompareTo(newVal) == 0)
                                {
                                    bSet = false;
                                }
                            }
                    ...
                    if (bSet)
                    {
                        dest.SetValue(obj, ordinal, newVal);
                    }
                }
			}
		}
