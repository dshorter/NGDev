	...
	public class DataException : System.Data.DataException
	{
		...
		public int? Number
		{
			get
			{
                ...
                // BVChanges: because of invalid cast it needs to check if result is int?
			    object result = _dbManager == null
			                        ? null
			                        : _dbManager.DataProvider.Convert(InnerException, ConvertType.ExceptionToErrorNumber);
                if (result == null)
                    return null;

			    int number;
			    return int.TryParse(result.ToString(), out number) 
                    ? (int?) number 
                    : null;
			}
		}
