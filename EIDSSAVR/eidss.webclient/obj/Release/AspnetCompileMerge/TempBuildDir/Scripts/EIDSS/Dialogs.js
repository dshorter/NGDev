...
var bvPopup = {
    ...
    showDefault: function (url, title, formNumber, width, heigth, onActivate) {
        bvCheckSession.check(function() {
            ...
            windowFacade.initAndShowModal(bvPopup.defaultName, url, title, formNumber, width, heigth, onActivate);
        });
    },

    showDefaultContent: function (content, title, formNumber, width, heigth, popupName) {
        bvCheckSession.check(function () {
            if (width < bvPopup.minWidth) {
                width = bvPopup.minWidth;
            }
            ...
            if (popupName == undefined) {
                popupName = bvPopup.defaultName;
            }
            windowFacade.initAndShowModalContent(popupName, content, title, formNumber, width, heigth);
        });
    },
    ...
    getDefaultWindowDataWithoutChanging: function () {
        var f = $("#" + bvPopup.defaultName + " .popupContent form");
        var data = f.serialize(true);

        $.each($("#" + bvPopup.defaultName + " .popupContent form input[type=checkbox]")
            .filter(function (idx) {
...
var bvTranslation = (function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 0) {
                    ...
                    bvDialog.showError(EIDSS.BvMessages['msgNotPossibleToSave']);
                    e.preventDefault();
                } else {
    ...
    return {
        ...
        /*onErrorTranslation: function (e) {
        },
        onSyncTranslation: function (e) {
        },
        onSaveChangesTranslation: function (e) {
        },
        */
    };
})();
