var windowFacade = (function () {
    ...
    function initModalWithHtml(id, htmlContent, title, width, heigth, onClose) {
        ...
        $("<div id='" + id + "'></div>").insertAfter('.detailPageContentReport');
        var control = $("#" + id);
    ...
    function getModal(id, url, title, formNumber, width, heigth) {
    ...
    function getModalContent(id, content, title, formNumber, width, heigth) {
        var modalWindow = getData($("#" + id));
        width = !width ? defaultSize.width : width;
        heigth = !heigth ? defaultSize.heigth : heigth;

        if (!modalWindow) {
            modalWindow = initModalWithHtml(id, content, title, width, heigth);
            modalWindow = getData(modalWindow);
        }
        else {
            modalWindow.setOptions({
    ...
    function centerWindowFix(id) {
        ...
        if (popupWindow.length > 0) {
            ...
            var popupWindowHeight = popupWindow.height();
    ...
    return {
        ...
        initAndShowModal: function (id, url, title, formNumber, width, heigth, onActivate) {
            var modalWindow = getModal(id, url, title, formNumber, width, heigth);
            if (onActivate != undefined) {
                modalWindow.bind("refresh", onActivate);
            }
            modalWindow.center().open();
        },
        initAndShowModalContent: function (id, content, title, formNumber, width, heigth) {
            var modalWindow = getModalContent(id, content, title, formNumber, width, heigth);
            modalWindow.center().open();
        },
