    ...
    public class PermissionParser
    {
        public static List<MenuItem> GetPermittedIconMenu(XDocument document, eidss.model.Core.EidssUserContext context)
        {
            ...
            foreach (var i in items)
            {
                ...
                if (i.Attribute("allowCustomization") != null && !string.IsNullOrEmpty(i.Attribute("allowCustomization").Value) && long.TryParse(i.Attribute("allowCustomization").Value, out customization) && eidss.model.Core.EidssSiteContext.Instance.CustomizationPackageID != customization)
                {
                    continue;
                }
        ...
        public static List<MenuCategory> GetPermittedMenu(XDocument document, eidss.model.Core.EidssUserContext context)
        {
            ...
            foreach (var i in items)
            {
                ...
                foreach (var e in i.Descendants("item"))
                {
                    ...
                    if (e.Attribute("allowCustomization") != null && !string.IsNullOrEmpty(e.Attribute("allowCustomization").Value) && long.TryParse(e.Attribute("allowCustomization").Value, out customization) && eidss.model.Core.EidssSiteContext.Instance.CustomizationPackageID != customization)
                    {
                        isActive = false;
                        continue;
                    }
