...
// ReSharper disable InconsistentNaming
        ...
        private string FindMod10Digit(string input)
        {
            ...
            int M10StringLength = input.Length;
            ...
            int remainder = M10WeightedTotal % 10;
            if (remainder == 0)
            {
        ...
        private string Code128TildeProcessing(string InputValue)
        {
            ...
            {
                if (InputValue.Substring(idx, 1).Equals("~") && ((lenDataToEncode - idx) > 3))
                {
                    //We are either doing mod 10 processing or we are doing regular tilde processing to pull the ascii value
                    if (InputValue.Substring(idx + 1, 1).Equals("m")) //we are doing mod 10
                    {
                        try
                        {
                            int tempValue = Convert.ToInt32(InputValue.Substring(idx + 2, 2));
                            if (tempValue > 0 && tempValue <= idx)
                                //make sure that they aren't going over the number of characters left in the string
                            {
                                ...
                                {
                                    if (IsNumber(stringToCheck.Substring(jdx - 1, 1)))
                                    {
                                ...
                                if (stringToCalculate.Length == tempValue)
                                {
                                    ...
                                    var revString = new StringBuilder();
                    ...
                    else if (InputValue.Substring(idx + 1, 1).Equals("~"))
                    {
        ...
        /// <param name="applyTilde">whether or not tildes are processed as special escape characters</param>
        /// <returns></returns>
        public string Code128(string inputValue, bool applyTilde=false)
        {
            ...
            var EncodingType = enuEncodingType.EncodingA; //type of character encodeing that we are doing

            string ActualInputValue = applyTilde 
                ? Code128TildeProcessing(inputValue)
                : inputValue;

            //Here we select character set A, B or C for the START character
            int LenDataToEncode = ActualInputValue.Length;
            ...
            char[] ChrArray = ActualInputValue.ToCharArray(0, 1);
            AscCharacter = ChrArray[0];
            ...
            if (LenDataToEncode > 4 && IsNumber(ActualInputValue.Substring(0, 4)))
            {
            ...
            char OutStopCharacter = (char) ASCII_STOP128;
            ...
            for (idx = 0; idx < LenDataToEncode; idx++)
            {
                ...
                bool AsciiFnc1Found = false; //Determines logic path in for loop if the FNC1 character is found
                ...
                if (((idx <= (LenDataToEncode - 4)) && IsNumber(ActualInputValue.Substring(idx, 1)) &&
                     IsNumber(ActualInputValue.Substring(idx + 1, 1)) &&
                     IsNumber(ActualInputValue.Substring(idx, 4))) ||
                    ((idx < LenDataToEncode - 1) && IsNumber(ActualInputValue.Substring(idx, 1)) &&
                     IsNumber(ActualInputValue.Substring(idx + 1, 1)) && (EncodingType == enuEncodingType.EncodingC))
                    && (AsciiFnc1Found == false))
                {
                    ...
                    string CurrentChars = ActualInputValue.Substring(idx, 2); //This is the character(s) that we are currently evaluating in the for loop
                    AscCharacter = CurrentChars == DOUBLE_ZEROES 
                        ? ASCII_SPACE_CHAR 
                        : Convert.ToInt32(CurrentChars);
            ...
            int WeightedTotal = OutStartCharacter - 100;
            ...
            int iCheckDigitValue = WeightedTotal % CHECK_DIGIT_MOD;
            ...
            char OutCheckCharacter = (char) iCheckDigitValue;
        ...
        public string Code128HR(string InputValue, bool applyTilde)
        {
            ...
            for (int i = 0; i < charDataToEncode.Length; i++)
            {
                ...
                else if ((charDataToEncode[i] >= 32) && (charDataToEncode[i] <= 126))
                {
                    //everything else that is a printable ASCII charcter
                    sbReturnString.Append(ActualInputValue.Substring(i, 1));
                }
                else if (charDataToEncode[i] < 32)
                {
        ...
        public string Code128A(string InputValue)
        {
            ...
            char Code128AStartChar = (char) ASCII_START128_A;
        ...
        public string Code128B(string InputValue)
        {
            ...
            char Code128BStartChar = (char) ASCII_START128_B;
            ...
            //Convert the ASCII value to the character representation.
            char Code128BCheckDigit = (char) iCurrentChar;
        ...
        public string Code128C(string InputValue)
        {
            ...
            char Code128CStartChar = (char) ASCII_START128_C;
        ...
        public string Codabar(string InputValue)
        {
            ...
            int idx; //for loop coutner

            int LenOfData = InputValue.Length;
            ...
            {
                string CurrentChar = InputValue.Substring(idx, 1); //Current character we are evaluating 
        ...
        public string Code39mod43(string InputValue)
        {
            ...
            for (idx = 0; idx < LenOfData; idx++)
            {
                ...
                else if (iCurrentChar == 32)
                    /* To print the barcode symbol representing a space you will to type or print "=" (the equal character) instead of a space character.*/
                {
                    iCurrentChar = 61;
                }
        ...
        public string EAN8(string InputValue)
        {
            ...
            for (idx = 0; idx < LenOfData; idx++)
            {
                ...
                {
                    case 1:
                        /* For the first character print the normal guard pattern and then the barcode without the human readable character */
        ...
        public string MSI(string InputValue)
        {
            ...
            int weightedTotal = iOddNumbers + EvenNumbers;
        ...
        public string UPCA(string InputValue)
        {
            ...
            string DataToPrint = "";
                //This is the actual encoded data once we have formatted the Input Value. This will be the function return value
        ...
        public string UPCe(string inputValue)
        {
            ...
            var eanAddOnToPrint = new StringBuilder();
            ...
            for (int idx = 0; idx < inputValue.Length; idx++)
            {
                ...
                if (Char.IsDigit(inputValue, idx)) //Make sure we only have numbers
                {
                    actualInputValue += inputValue.Substring(idx, 1);
                }
            }
                ...
                for (int I = 1; I <= actualInputValue.Length; I++)
                {
                    ...
                    switch (I)
                    {
                        ...
                        case 6: //Print the SPECIAL guard pattern and check character		
                            ...
                            if (D12 > 4)
                            {
                                sbOutput.Append((char) (D12.ToString().ToCharArray()[0] + 64));
                            }
                            if (D12 < 5)
                            {
                                sbOutput.Append((char) (D12.ToString().ToCharArray()[0] + 37));
                            }
                            break;
                    } //end switch
                } //end for loop 
            ...
            sbOutput.Append(eanAddOnToPrint);
        ...
        public void PrintBarCode(string textFont, string encodedText, float textFontSize)
        {
            ...
            printDoc.PrintPage += PrintDocHandler;
