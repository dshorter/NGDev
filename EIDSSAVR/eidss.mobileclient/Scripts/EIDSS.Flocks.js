...
function onClinicalSignsClick(idfCase, name) {
    var selectedId = GetSelectGridItemId("FlocksList");
...
function onSaveAndCloseSpecies(params) {
    ...
    var lang = GetSiteLanguage();
    var postUrl = "/" + lang + "/Flock/SpeciesDetails";
    $.ajax({
        url: postUrl,
        cache: false,
        dataType: "json",
...
function CloseSpecies(idfCase) {
    var lang = GetSiteLanguage();
    var url = "/" + lang + "/VetCase/FlockDetails?id=" + idfCase;
...
function onEditSpeciesClick(idfCase, name) {
    var selectedId = GetSelectGridItemId("FlocksList");
    var lang = GetSiteLanguage();
    var postUrl = "/" + lang + "/Flock/SpeciesDetails?rootKey=" + idfCase + "&name=" + name + "&idfSpecies=" + selectedId;
    document.location.href = postUrl;
}
