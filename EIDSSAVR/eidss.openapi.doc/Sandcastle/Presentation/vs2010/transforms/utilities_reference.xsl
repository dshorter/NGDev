...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--g_topicGroup = <xsl:value-of select="$g_topicGroup"/>;
    g_apiGroup = <xsl:value-of select="$g_apiGroup"/>;
    g_apiSubGroup = <xsl:value-of select="$g_apiSubGroup"/>;
    g_apiSubSubGroup = <xsl:value-of select="$g_apiSubSubGroup"/>;-->
    <xsl:choose>
      <xsl:when test="$g_topicGroup='api' and $g_apiGroup='namespace'">
        <xsl:apply-templates select="/document/comments/summary"/>
      </xsl:when>
