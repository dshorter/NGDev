...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:if test="count(*) > 0">
			<xsl:call-template name="t_putSectionInclude">
				...
				<xsl:with-param name="p_content">
          <xsl:choose>
            <xsl:when test="/document/reference/containers/namespace/@api='N:eidss.openapi.web.Controllers' and /document/reference/containers/type/apidata/@name">
              <!--xsl:call-template name="t_putCodeSection">
                <xsl:with-param name="p_text" xml:space="preserve">http://hostname/api/<xsl:value-of select="substring-before(/document/reference/containers/type/apidata/@name, 'Controller')"/><xsl:choose><xsl:when test="/document/reference/apidata/@name='List'">/<xsl:value-of select="/document/reference/apidata/@name"/><xsl:for-each select="/document/reference/parameters/parameter">?<xsl:value-of select="@name"/>={...}</xsl:for-each></xsl:when><xsl:otherwise><xsl:for-each select="/document/reference/parameters/parameter"><xsl:choose><xsl:when test="type/@api='T:System.Int64'">/<xsl:value-of select="@name"/></xsl:when><xsl:otherwise> [<xsl:value-of select="@name"/> in body]</xsl:otherwise></xsl:choose></xsl:for-each></xsl:otherwise></xsl:choose></xsl:with-param>
              </xsl:call-template-->
              <xsl:call-template name="t_putCodeSection">
                <xsl:with-param name="p_text" xml:space="preserve"><xsl:value-of select="/document/comments/remarks"/></xsl:with-param>
              </xsl:call-template>
            </xsl:when>
            <xsl:when test="/document/reference/containers/namespace/@api='N:eidss.openapi.web.Controllers' and not(/document/reference/containers/type/apidata/@name)">
            </xsl:when>
            <xsl:otherwise>
