    ...
    <xsl:param name="p_text" select="." />
		...
		<xsl:element name="mtps:CodeSnippet"
			...
			<xsl:attribute name="ContainsMarkup">
				<xsl:choose>
					<xsl:when test="starts-with(normalize-space($p_text),'http://hostname/api/')">
						<xsl:value-of select="'true'"/>
					</xsl:when>
					<xsl:when test="starts-with(normalize-space($p_text),'HTTP')">
						<xsl:value-of select="'true'"/>
					</xsl:when>
          <xsl:when test="starts-with(normalize-space($p_text),'@@_')">
            <xsl:value-of select="'true'"/>
          </xsl:when>
          <xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="starts-with(normalize-space($p_text),'http://hostname/api/')">
					<xsl:element name="pre" namespace="{$xhtml}" xml:space="preserve"><xsl:value-of select="$p_text"/></xsl:element>
				</xsl:when>
				<xsl:when test="starts-with(normalize-space($p_text),'HTTP')">
					<xsl:element name="pre" namespace="{$xhtml}" xml:space="preserve"><xsl:value-of select="$p_text"/></xsl:element>
				</xsl:when>
