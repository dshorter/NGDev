...
namespace AUM.Core.Transactions
{
    ...
    public class TxFileManager : IFileManager
    {
        ...
        public void Copy(string sourceFileName, string destFileName, bool overwrite)
        {
            if (IsInTransaction())
            {
                EnlistOperation(new CopyOperation(sourceFileName, destFileName));
            }
            else
            {
        ...
        [ThreadStatic]
        private static Dictionary<string, TxEnlistment> m_Enlistments;

        private static readonly object m_EnlistmentsLock = new object();
        ...
        private static void EnlistOperation(IRollbackableOperation operation)
        {
            ...
            lock (m_EnlistmentsLock)
            {
                if (m_Enlistments == null)
                {
                    m_Enlistments = new Dictionary<string, TxEnlistment>();
                }

                TxEnlistment enlistment;
                if (!m_Enlistments.TryGetValue(tx.TransactionInformation.LocalIdentifier, out enlistment))
                {
                    enlistment = new TxEnlistment(tx);
                    m_Enlistments.Add(tx.TransactionInformation.LocalIdentifier, enlistment);
                }
